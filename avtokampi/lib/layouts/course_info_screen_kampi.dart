...
class _CourseInfoScreenState extends State<CourseInfoScreen>
    ...
    List<Image> getSlikeZaKamp(int kampId) {
        List<Image> slike = [];
        for (Slika slika in globals.slike) {
            if (slika.avtokamp == kampId) {
                slike.add(Image.network(slika.url, fit: BoxFit.cover));
            }
        }
        if (slike.isEmpty) {
            slike.add(Image.network(globals.slike[0].url, fit: BoxFit.cover));
        }
        return slike;
    }
    ...
    void initState() {
        ...
        setData();
        slika = avtokampElement.imagePath;
